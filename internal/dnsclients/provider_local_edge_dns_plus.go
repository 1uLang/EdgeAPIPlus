...
func (this *LocalEdgeDNSProvider) GetRecords(domain string) (records []*dnstypes.Record, err error) {
	var tx *dbs.Tx
	domainId, err := nameservers.SharedNSDomainDAO.FindDomainIdWithName(tx, this.clusterId, 0, domain)
	if err != nil {
		return nil, err
	}
	if domainId == 0 {
	...
	size := int64(1000)
	for {
		result, err := nameservers.SharedNSRecordDAO.ListEnabledRecords(tx, domainId, "", "", "", false, false, false, false, false, false, offset, size)
		if err != nil {
			return nil, err
		}
		if len(result) == 0 {
