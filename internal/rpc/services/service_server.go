...
// CountAllEnabledServersWithGMCertId 计算使用某个国密证书的服务数量
func (this *ServerService) CountAllEnabledServersWithGMCertId(ctx context.Context, req *pb.CountAllEnabledServersWithGMCertIdRequest) (*pb.RPCCountResponse, error) {
	// 校验请求
	_, userId, err := this.ValidateAdminAndUser(ctx, true)
	if err != nil {
		return nil, err
	...
	policyIds, err := models.SharedSSLPolicyDAO.FindAllEnabledPolicyIdsWithGmCertId(tx, req.GmCertId)
	if err != nil {
		return nil, err
	}
...
// FindAllEnabledServersWithGMCertId 查找使用某个国密证书的所有服务
func (this *ServerService) FindAllEnabledServersWithGMCertId(ctx context.Context, req *pb.FindAllEnabledServersWithGMCertIdRequest) (*pb.FindAllEnabledServersWithGMCertIdResponse, error) {
	// 校验请求
	_, userId, err := this.ValidateAdminAndUser(ctx, true)
	if err != nil {
		return nil, err
	...
	if len(policyIds) == 0 {
		return &pb.FindAllEnabledServersWithGMCertIdResponse{Servers: nil}, nil
	}
	...
	return &pb.FindAllEnabledServersWithGMCertIdResponse{Servers: result}, nil
}
