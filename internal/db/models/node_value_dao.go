...
// FindLatestClusterValue 集群下所有节点的最近十分钟的数据
func (this *NodeValueDAO) FindLatestClusterValue(tx *dbs.Tx, role string, clusterId int64, item string, minute time.Duration) (result []*NodeValue, err error) {
	now := time.Now()
	end := timeutil.FormatTime("YmdHi", now.Unix())
	start := timeutil.FormatTime("YmdHi", now.Add(-1*minute).Unix())
	query := this.Query(tx).
		Attr("role", role).
		Attr("item", item).
		Gte("minute", start).
...
// FindLatestAllValue 平台下所有节点的最近十分钟的数据
func (this *NodeValueDAO) FindLatestAllValue(tx *dbs.Tx, role string, item string, minute time.Duration) (result []*NodeValue, err error) {
	now := time.Now()
	end := timeutil.FormatTime("YmdHi", now.Unix())
	start := timeutil.FormatTime("YmdHi", now.Add(-1*minute).Unix())
	query := this.Query(tx).
