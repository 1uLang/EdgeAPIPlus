...
func (this *NSNodeDAO) ComposeNodeConfig(tx *dbs.Tx, nodeId int64) (*dnsconfigs.NSNodeConfig, error) {
	...
	// email
	config.Email = cluster.Email

	// hosts
	config.Hosts = cluster.DecodeHosts()

	// soa
	config.SOA = cluster.DecodeSOAConfig()
	config.SOASerial = types.Uint32(cluster.SoaSerial)

	// answer config
	config.Answer = cluster.DecodeAnswerConfig()
